...
static float absmax_scale(const std::vector<float>& v, float qmax) {
  float m=0.f; for (auto x: v) m = std::max(m, std::fabs(x));
  if (m < 1e-12f) m = 1e-12f;
  return m / qmax;
}
static int8_t q8(float x, float s) { int q = (int)llround(x/s); q = clamp(q, -128, 127); return (int8_t)q; }
static int8_t q4(float x, float s) { int q = (int)llround(x/s); q = clamp(q, -8, 7); return (int8_t)q; }
static uint8_t qb(float x) { return (x < 0.f) ? 0u : 1u; } // 0=>-1,1=>+1

static void tick(Vqnn_accel_top* dut, vluint64_t& t) {
  dut->clk = 0; dut->eval(); t++;
  dut->clk = 1; dut->eval(); t++;
}
static int32_t u32_to_s32(uint32_t u) { return (int32_t)u; }

int main(int argc, char** argv) {
  Verilated::commandArgs(argc, argv);
  auto* dut = new Vqnn_accel_top;
  ...
  for (int i=0;i<IN_DIM;i++) x[i] = frand(-1.f, 1.f);
  for (int o=0;o<OUT_DIM;o++) {
    bias[o] = (int32_t)llround(frand(-50.f, 50.f));
    for (int i=0;i<IN_DIM;i++) w[o*IN_DIM+i] = frand(-1.f, 1.f);
  }

  auto run_mode = [&](int prec_mode, const char* name) {
    float qmax = (prec_mode==0) ? 127.f : (prec_mode==1 ? 7.f : 1.f);
    float sx = absmax_scale(x, qmax);

    std::vector<int8_t> x8(IN_DIM), x4(IN_DIM);
    std::vector<uint8_t> xb(IN_DIM);
    for (int i=0;i<IN_DIM;i++){ x8[i]=q8(x[i],sx); x4[i]=q4(x[i],sx); xb[i]=qb(x[i]); }
    ...
    for (int o=0;o<OUT_DIM;o++){
      ...
      float sw = absmax_scale(row, qmax);
      for (int i=0;i<IN_DIM;i++){
        w8[o*IN_DIM+i]=q8(w[o*IN_DIM+i],sw);
        w4[o*IN_DIM+i]=q4(w[o*IN_DIM+i],sw);
        wb[o*IN_DIM+i]=qb(w[o*IN_DIM+i]);
      }
    }
    ...
    dut->layer_prec = (uint8_t)prec_mode;
    ...
    while(!dut->done){
      tick(dut,t);
      if(++guard > (OUT_DIM*IN_DIM + OUT_DIM + 200)){
